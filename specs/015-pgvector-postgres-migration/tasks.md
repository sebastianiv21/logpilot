...
**Independent Test**: Start the app with only `DATABASE_URL` set (no `QDRANT_URL`, no `DATA_DIR`), create a session, upload a log file, confirm session and upload summary appear in PostgreSQL, confirm embedding rows exist in `log_chunk_embeddings` table.
