...
2. **Upload flow**: POST `/sessions/{id}/logs/upload` with a `.zip` file; response must include `uploaded_file_name` (the original filename).
3. **Summary**: GET `/sessions/{id}/upload-summary` returns last result including `uploaded_file_name` (null for legacy rows).

**Verify**:
- Create a session, POST upload with file `test.zip`; response has `uploaded_file_name: "test.zip"`.
- GET same session’s upload-summary; response has `uploaded_file_name: "test.zip"`.
...
2. **Logs & metrics section**: Single heading "Logs & metrics"; contains file input, upload button, latest upload summary (with file name when present), and "Open in Grafana" (no helper text "Opens in a new tab; updates when you switch sessions.").
3. **Upload summary**: Show loading indicator (spinner/skeleton) while upload-summary query is loading; display file name from API when present.
...
- After upload: summary shows status and file name; reports become available.
- Refresh: summary (and file name) from API; reports available if summary exists with success/partial.
...
- [ ] Latest upload summary shows uploaded file name (when available from API).
